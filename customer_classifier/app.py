...
# 테마 색상 팔레트
# ─────────────────────────────────────────────
# 분류 색상은 CVD(색각이상) 검증을 통과한 값 (OKLCH 기반, deutan/protan ΔE≥15):
#   실패수요=적색, 가치수요=시안 — 의미 축 2개만 유채색.
#   기타/분류실패는 의도적 회색(강조 해제) — 분석 스토리의 주인공이 아니므로.
_DARK = {
    "cat": {
        "실패수요": "#D5584F",
        "가치수요": "#0096AF",
...
with tab_classify:
    ...
    # 주의: 여기서 st.stop()을 쓰면 스크립트 전체가 중단되어
    # 3·4단계 탭까지 비어버린다. 반드시 조건 분기로만 가드할 것.
    if std_df is None:
        st.info("1단계 탭에서 데이터를 먼저 불러와 주세요.")
    elif not model:
        st.warning(
            "사이드바에서 Ollama 모델을 선택해주세요. "
            "기존 분류 결과가 있다면 3단계 탭에서 계속 확인할 수 있습니다."
        )
    else:
...
# 탭 3 — 분석 대시보드 (결과·검토)
#
# 레이어 스토리: 위에서 아래로 질문에 답한다.
#   ① 얼마나 문제인가   (헤드라인 KPI)
#   ② 무엇이 문제인가   (수요 구조 → Pain Point 순위)
#   ③ 언제 생겼나       (월별 추이 — 급증 탐지)
#   ④ 어디서 생겼나     (상품 × 실패 핫스팟)
#   ⑤ 여정에 어떤 영향  (구매 전환 — 여정 데이터가 있을 때)
#   ⑥ 개별 사례 확인    (고심각 사례 → 검토·수정)
#   ⑦ 산출물            (다운로드 → 4단계 전략 리포트)
# ══════════════════════════════════════════════
with tab_result:
    result_df = st.session_state.get("result_df")
    ...
    if result_df is None:
        ...
        def _tidy(fig, height=None, legend_h=True):
            """차트 공통 마감: 배경·격자 절제, 여백 축소, 가로 범례"""
            fig.update_layout(
                paper_bgcolor=PL["paper_bgcolor"],
                plot_bgcolor=PL["plot_bgcolor"],
                font_color=PL["font_color"],
                margin={"l": 10, "r": 10, "t": 84 if legend_h else 48, "b": 10},
                title={"font": {"size": 14}, "y": 0.97, "yanchor": "top",
                       "x": 0, "xanchor": "left"},
            )
            if legend_h:
                fig.update_layout(legend={"orientation": "h", "yanchor": "bottom",
                                          "y": 1.0, "xanchor": "left", "x": 0,
                                          "traceorder": "normal"})
            if height:
                fig.update_layout(height=height)
            fig.update_xaxes(gridcolor=PL["gridcolor"], zeroline=False, automargin=True)
            fig.update_yaxes(gridcolor=PL["gridcolor"], zeroline=False, automargin=True)
            return fig
        ...
        # 실패수요 건별 심각도 (세부분류 특성 + 감정 강도, 1~5) — 리포트와 동일 기준
        if len(fd):
            fd["심각도"] = [rb.item_severity(s, t) for s, t in zip(fd["세부분류"], fd["text"])]
        has_journey = "journey_stage" in result_df.columns

        # ────────────────────────────────────────────
        # ① 얼마나 문제인가 — 헤드라인
        # ────────────────────────────────────────────
        st.markdown('<div class="section-title">① 얼마나 문제인가 — 헤드라인</div>', unsafe_allow_html=True)
        st.caption(
            "이 대시보드는 위에서 아래로 **얼마나 → 무엇이 → 언제 → 어디서 → 여정 영향 → 개별 사례 → 산출물** "
            "순서로 읽도록 배치되어 있습니다."
        )

        fail_n   = int(counts.get("실패수요", 0))
        fail_pct = fail_n / total * 100 if total else 0.0
        sev_hi_n = int((fd["심각도"] >= 4).sum()) if len(fd) else 0
        top_pain = fd["세부분류"].value_counts()
        top_pain_name = top_pain.index[0] if len(top_pain) else "—"
        top_pain_n    = int(top_pain.iloc[0]) if len(top_pain) else 0
        review_n = int(
            (pd.to_numeric(result_df["확신도"], errors="coerce").fillna(0) < confidence_threshold).sum()
            + counts.get("분류실패", 0)
        )

        if has_journey:
            _pre = result_df[~result_df["journey_stage"].astype(str).str.startswith("구매후")]
            conv = pd.to_numeric(_pre.get("구매전환"), errors="coerce").fillna(0)
            kpi5 = ("구매 전 문의 전환율", f"{conv.mean() * 100:.1f}%",
                    f"구매 전 문의 {len(_pre):,}건 기준", _C("accent"))
        else:
            val_pct = counts.get("가치수요", 0) / total * 100 if total else 0.0
            kpi5 = ("가치수요 비율", f"{val_pct:.1f}%",
                    f"{counts.get('가치수요', 0):,}건 — 영업 기회", cat_colors["가치수요"])

        kpis = [
            ("실패수요 비율", f"{fail_pct:.1f}%", f"{fail_n:,}건 / 전체 {total:,}건", cat_colors["실패수요"]),
            ("고심각 Pain Point", f"{sev_hi_n:,}건", "심각도 4 이상 (시급 대응)", cat_colors["실패수요"]),
            ("최다 실패 유형", top_pain_name, f"{top_pain_n:,}건", _C("text")),
            kpi5,
            ("검토 필요", f"{review_n:,}건", f"확신도 {confidence_threshold} 미만 + 분류실패", _C("warn")),
        ]
        kpi_cols = st.columns(5)
        for col_w, (label, value, sub, color) in zip(kpi_cols, kpis):
            with col_w:
                st.markdown(
                    f'<div class="metric-card">'
                    f'<div class="m-label">{label}</div>'
                    f'<div class="m-value" style="color:{color}">{value}</div>'
                    f'<div class="m-pct">{sub}</div></div>',
                    unsafe_allow_html=True,
                )

        # ────────────────────────────────────────────
        # ② 무엇이 문제인가 — 수요 구조와 Pain Point 순위
        # ────────────────────────────────────────────
        st.markdown('<div class="section-title">② 무엇이 문제인가 — 수요 구조와 Pain Point 순위</div>', unsafe_allow_html=True)

        # 수요 구조: 100% 한 줄 스택바 (파이 대신 — 비율 비교가 정확)
        order = ["실패수요", "가치수요", "기타", "판단보류", "분류실패"]
        ...
        for c in order:
            ...
            pct = n / total * 100
            fig_struct.add_bar(
                y=["수요 구조"], x=[n], name=c, orientation="h",
                marker={"color": cat_colors[c],
                        "line": {"color": PL["paper_bgcolor"], "width": 2}},
                text=f"{c} {pct:.0f}%" if pct >= 8 else "",
                textposition="inside", insidetextanchor="middle",
                hovertemplate=f"{c}: {n:,}건 ({pct:.1f}%)<extra></extra>",
            )
        fig_struct.update_layout(barmode="stack", height=130, showlegend=True,
                                 title=f"수요 구조 (전체 {total:,}건)")
        fig_struct.update_xaxes(visible=False)
        fig_struct.update_yaxes(visible=False)
        st.plotly_chart(_tidy(fig_struct, height=150), use_container_width=True, theme=None)
        ...
        with sub_col1:
            if len(fd) > 0:
                sub_counts = (
                    fd.groupby("세부분류")
                    .agg(건수=("세부분류", "size"), 평균심각도=("심각도", "mean"), 고심각=("심각도", lambda s: int((s >= 4).sum())))
                    .reset_index()
                    .sort_values(["건수", "평균심각도"], ascending=False)
                )
                sub_counts["설명"]   = sub_counts["세부분류"].map(FAILURE_SUBCATEGORIES).fillna("")
                sub_counts["비율"]   = (sub_counts["건수"] / len(fd) * 100).round(1)
                sub_counts["평균심각도"] = sub_counts["평균심각도"].round(1)

                fig_fail = go.Figure(go.Bar(
                    x=sub_counts["건수"], y=sub_counts["세부분류"], orientation="h",
                    marker={"color": cat_colors["실패수요"],
                            "line": {"color": PL["paper_bgcolor"], "width": 2}},
                    text=sub_counts["건수"], textposition="outside",
                    customdata=sub_counts[["비율", "평균심각도", "고심각"]],
                    hovertemplate="%{y}: %{x:,}건 (%{customdata[0]}%)<br>"
                                  "평균심각도 %{customdata[1]} · 고심각 %{customdata[2]}건<extra></extra>",
                ))
                fig_fail.update_layout(title=f"실패수요 세부분류 — 개선 과제 후보 ({len(fd):,}건)", showlegend=False)
                fig_fail.update_yaxes(autorange="reversed")
                st.plotly_chart(_tidy(fig_fail, height=380, legend_h=False), use_container_width=True, theme=None)

                view = sub_counts.copy()
                view.insert(0, "우선순위", range(1, len(view) + 1))
                view["비율"] = view["비율"].astype(str) + "%"
                st.dataframe(
                    view[["우선순위", "세부분류", "설명", "건수", "비율", "평균심각도", "고심각"]],
        ...
        with sub_col2:
            if len(vd) > 0:
                sub_v = vd["세부분류"].value_counts().reset_index()
                sub_v.columns = ["세부분류", "건수"]
                sub_v["설명"] = sub_v["세부분류"].map(VALUE_SUBCATEGORIES).fillna("")
                sub_v["비율"] = (sub_v["건수"] / len(vd) * 100).round(1)

                fig_val = go.Figure(go.Bar(
                    x=sub_v["건수"], y=sub_v["세부분류"], orientation="h",
                    marker={"color": cat_colors["가치수요"],
                            "line": {"color": PL["paper_bgcolor"], "width": 2}},
                    text=sub_v["건수"], textposition="outside",
                    customdata=sub_v[["비율"]],
                    hovertemplate="%{y}: %{x:,}건 (%{customdata[0]}%)<extra></extra>",
                ))
                fig_val.update_layout(title=f"가치수요 세부분류 — 대응 기회 ({len(vd):,}건)", showlegend=False)
                fig_val.update_yaxes(autorange="reversed")
                st.plotly_chart(_tidy(fig_val, height=380, legend_h=False), use_container_width=True, theme=None)

                view_v = sub_v.copy()
                view_v.insert(0, "기회순위", range(1, len(view_v) + 1))
                view_v["비율"] = view_v["비율"].astype(str) + "%"
                st.dataframe(
                    view_v[["기회순위", "세부분류", "설명", "건수", "비율"]],
                    use_container_width=True, hide_index=True,
                )
            else:
        ...
        # ────────────────────────────────────────────
        df_dated = None
        if "written_at" in result_df.columns and result_df["written_at"].astype(str).str.strip().ne("").any():
            _d = result_df.copy()
            _d["날짜"] = pd.to_datetime(_d["written_at"], errors="coerce")
            _d = _d.dropna(subset=["날짜"])
            if len(_d) > 0 and _d["날짜"].dt.to_period("M").nunique() >= 2:
                _d["월"] = _d["날짜"].dt.to_period("M").astype(str)
                df_dated = _d

        if df_dated is not None:
            st.markdown('<div class="section-title">③ 언제 생겼나 — 월별 추이</div>', unsafe_allow_html=True)
            st.caption("특정 월에 급증한 유형은 이벤트성 원인(프로모션·시스템 배포·물류 이슈)을 의심하세요.")
            t_col1, t_col2 = st.columns(2)

            with t_col1:
                trend = (
                    df_dated[df_dated["분류"].isin(["실패수요", "가치수요"])]
                    .groupby(["월", "분류"]).size().reset_index(name="건수")
                )
                fig_trend = go.Figure()
                for c in ["실패수요", "가치수요"]:
                    seg = trend[trend["분류"] == c]
                    if len(seg) == 0:
                        continue
                    fig_trend.add_scatter(
                        x=seg["월"], y=seg["건수"], name=c, mode="lines+markers",
                        line={"color": cat_colors[c], "width": 2}, marker={"size": 8},
                        hovertemplate="%{x} " + c + ": %{y:,}건<extra></extra>",
                    )
                fig_trend.update_layout(title="실패수요 vs 가치수요 월별 건수")
                fig_trend.update_xaxes(type="category")
                st.plotly_chart(_tidy(fig_trend, height=340), use_container_width=True, theme=None)

            with t_col2:
                fd_dated = df_dated[df_dated["분류"] == "실패수요"]
                top3 = fd_dated["세부분류"].value_counts().head(3).index.tolist()
                if len(fd_dated) and top3:
                    tr2 = (
                        fd_dated[fd_dated["세부분류"].isin(top3)]
                        .groupby(["월", "세부분류"]).size().reset_index(name="건수")
                    )
                    fig_top = go.Figure()
                    for i, sub in enumerate(top3):
                        seg = tr2[tr2["세부분류"] == sub]
                        emphasized = i == 0  # 최다 유형만 강조, 나머지는 회색 맥락
                        fig_top.add_scatter(
                            x=seg["월"], y=seg["건수"], name=sub, mode="lines+markers",
                            line={"color": cat_colors["실패수요"] if emphasized else _C("text_muted"),
                                  "width": 2.5 if emphasized else 2},
                            marker={"size": 8 if emphasized else 6},
                            hovertemplate="%{x} " + sub + ": %{y:,}건<extra></extra>",
                        )
                    fig_top.update_layout(title=f"실패수요 상위 유형 추이 (강조: {top3[0]})")
                    fig_top.update_xaxes(type="category")
                    st.plotly_chart(_tidy(fig_top, height=340), use_container_width=True, theme=None)
                else:
                    st.info("실패수요 추이를 그릴 데이터가 부족합니다.")

        # ────────────────────────────────────────────
        # ④ 어디서 생겼나 — 상품 × 실패 유형 핫스팟
        # ────────────────────────────────────────────
        if len(fd) > 0 and "product" in fd.columns and fd["product"].astype(str).str.strip().ne("").any():
            st.markdown('<div class="section-title">④ 어디서 생겼나 — 상품 × 실패 유형 핫스팟</div>', unsafe_allow_html=True)
            st.caption("특정 상품·유형 교차에 집중되면 상품 단위 원인(페이지 정보·공급·물류)일 가능성이 큽니다.")
            top_products = (
                fd["product"].astype(str).str.strip().replace("", pd.NA).dropna()
                .value_counts().head(8).index.tolist()
            )
            hot = fd[fd["product"].isin(top_products)]
            if len(hot) > 0:
                cross = pd.crosstab(hot["product"], hot["세부분류"])
                cross = cross.loc[cross.sum(axis=1).sort_values(ascending=False).index]
                fig_hot = px.imshow(
                    cross, text_auto=True, aspect="auto",
                    color_continuous_scale=[[0, PL["plot_bgcolor"]], [1, cat_colors["실패수요"]]],
                )
                fig_hot.update_layout(title="실패수요 건수 (상위 상품 × 세부분류)", coloraxis_showscale=False,
                                      xaxis_title=None, yaxis_title=None)
                fig_hot.update_xaxes(side="top")
                fig_hot = _tidy(fig_hot, height=110 + 42 * len(cross), legend_h=False)
                fig_hot.update_layout(margin={"t": 88})  # 상단 축 라벨과 제목 겹침 방지
                st.plotly_chart(fig_hot, use_container_width=True, theme=None)

        # ────────────────────────────────────────────
        # ⑤ 여정에 어떤 영향 — 구매 전환 (여정 연계 데이터일 때)
        # ────────────────────────────────────────────
        if has_journey:
            st.markdown('<div class="section-title">⑤ 여정에 어떤 영향 — 문의 전후 행동과 전환</div>', unsafe_allow_html=True)
            ...
            with j_col1:
                ...
                js["실패수요비중"] = js["여정단계"].map(fail_share)
                fig_js = go.Figure(go.Bar(
                    x=js["건수"], y=js["여정단계"], orientation="h",
                    marker={"color": _C("accent"),
                            "line": {"color": PL["paper_bgcolor"], "width": 2}},
                    text=js["건수"], textposition="outside",
                    customdata=js[["실패수요비중"]],
                    hovertemplate="%{y}: %{x:,}건<br>실패수요 비중 %{customdata[0]}%<extra></extra>",
                ))
                fig_js.update_layout(title="문의가 발생한 여정 단계", showlegend=False)
                fig_js.update_yaxes(autorange="reversed")
                st.plotly_chart(_tidy(fig_js, height=320, legend_h=False), use_container_width=True, theme=None)

            with j_col2:
                pre = result_df[~result_df["journey_stage"].astype(str).str.startswith("구매후")].copy()
                pre["구매전환"] = pd.to_numeric(pre.get("구매전환"), errors="coerce").fillna(0)
                conv_by_sub = (
                    pre.groupby("세부분류")
                    .agg(건수=("구매전환", "size"), 전환율=("구매전환", "mean"))
                    .query("건수 >= 5")
                    .sort_values("전환율", ascending=False)
                    .head(8)
                    .reset_index()
                )
                if len(conv_by_sub) > 0:
                    conv_by_sub["전환율%"] = (conv_by_sub["전환율"] * 100).round(1)
                    fig_conv = go.Figure(go.Bar(
                        x=conv_by_sub["전환율%"], y=conv_by_sub["세부분류"], orientation="h",
                        marker={"color": cat_colors["가치수요"],
                                "line": {"color": PL["paper_bgcolor"], "width": 2}},
                        text=conv_by_sub["전환율%"].astype(str) + "%", textposition="outside",
                        customdata=conv_by_sub[["건수"]],
                        hovertemplate="%{y}: 전환율 %{x}%<br>구매 전 문의 %{customdata[0]:,}건<extra></extra>",
                    ))
                    fig_conv.update_layout(title="구매 전 문의 → 구매 전환율 (세부분류별, 5건 이상)", showlegend=False)
                    fig_conv.update_yaxes(autorange="reversed")
                    fig_conv.update_xaxes(range=[0, max(100, conv_by_sub["전환율%"].max() * 1.15)])
                    st.plotly_chart(_tidy(fig_conv, height=320, legend_h=False), use_container_width=True, theme=None)
                else:
                    st.info("전환율을 계산할 구매 전 문의가 부족합니다 (세부분류별 5건 이상 필요).")

            # 답변 품질 → 전환 영향 (미니 스탯)
            if "a_fail_yn" in result_df.columns:
                pre = result_df[~result_df["journey_stage"].astype(str).str.startswith("구매후")].copy()
                pre["구매전환"] = pd.to_numeric(pre.get("구매전환"), errors="coerce").fillna(0)
                is_fail = pre["a_fail_yn"].astype(str).eq("fail")
                n_f, n_o = int(is_fail.sum()), int((~is_fail).sum())
                if n_f and n_o:
                    conv_o = pre.loc[~is_fail, "구매전환"].mean() * 100
                    conv_f = pre.loc[is_fail, "구매전환"].mean() * 100
                    a_col1, a_col2, a_col3 = st.columns(3)
                    for col_w, (label, value, sub, color) in zip(
                        [a_col1, a_col2, a_col3],
                        [
                            ("정상 답변 전환율", f"{conv_o:.1f}%", f"{n_o:,}건", _C("ok")),
                            ("실패 답변 전환율", f"{conv_f:.1f}%", f"{n_f:,}건 (죄송/안내불가 등)", _C("err")),
                            ("전환율 격차", f"{conv_o - conv_f:+.1f}%p", "답변 커버리지 개선의 기대효과", _C("text")),
                        ],
                    ):
                        with col_w:
                            st.markdown(
        ...
        # ────────────────────────────────────────────
        # ⑥ 개별 사례 확인 — 고심각 사례와 검토·수정
        # ────────────────────────────────────────────
        st.markdown('<div class="section-title">⑥ 개별 사례 확인 — 고심각 사례와 검토·수정</div>', unsafe_allow_html=True)

        if len(fd) > 0 and (fd["심각도"] >= 4).any():
            with st.expander(f"🚨 고심각 사례 우선 확인 (심각도 4 이상 · {sev_hi_n:,}건 중 상위 10건)", expanded=False):
                urgent = fd[fd["심각도"] >= 4].sort_values(
                    ["심각도", "확신도"], ascending=False
                ).head(10)
                show_cols = [c for c in ["심각도", "세부분류", "text", "inquiry_id", "product"] if c in urgent.columns]
        ...
        # ⑦ 산출물 — 다운로드와 다음 단계
        # ────────────────────────────────────────────
        st.markdown('<div class="section-title">⑦ 산출물 — 다운로드와 다음 단계</div>', unsafe_allow_html=True)
        ...
        with dl_col2:
            st.markdown(
                '<div class="info-box">검토를 마쳤다면 <b>🧭 4단계: 전략 리포트</b> 탭에서 '
                "개인정보 마스킹된 집계 리포트를 생성해 Claude 프로젝트로 전략을 도출하세요."
                "</div>",
                unsafe_allow_html=True,
            )
